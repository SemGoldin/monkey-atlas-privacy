...
```
**Пояснення:** `Rotate` додає обертання при кожному виклику, тож варто запускати його одноразово (наприклад, у реакції на кнопку) або масштабувати кут за часом.  
**Практика:** Створити платформу, збільшувати її масштаб при натисканні клавіші; додати `Mathf.Clamp` для обмеження масштабу (наприклад 0.5f–3f); обертати на 90° лише після натискання окремої клавіші.  
...
```
**Пояснення:** `TryGetComponent` робить один пошук компонента, але відразу повертає результат і перевірку на `null` в єдиному виклику, спрощуючи код.  
...
**Теорія:** Дані, що зберігаються як ассети, спільні між сценами.  
**Код-приклад:**
```csharp
using UnityEngine;
...
```
**Пояснення:** Створюємо ассет через меню, не потребує GameObject.  
**Практика:** Створити ассет PlayerStats, прочитати значення в скрипті руху.  
**Типові помилки:** Зміна полів у рантаймі прямо в ассеті → впливає на всі екземпляри.
